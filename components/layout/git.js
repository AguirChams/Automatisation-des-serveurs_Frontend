...
const UpdateScript = () => {
  ...
  const handleExecuteScript = async () => {
    try {
      const res = await axios.post('/api/git/executeCommand');
  ...
  useEffect(() => {
    ...
    const mqttTopic = 'executionTerminatedGit';
    ...
    const handleConnect = () => {
      ...
      client.subscribe('executionTerminatedGit', { qos: 0 }, (err) => {
        if (err) {
  ...
  return (
    ...
    <Col md={10}>
          <View style={{ display: 'block', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: '30%', marginLeft: '15%' }}>
            ...
            <Text style={{ textAlign: 'center', marginBottom: 20, color: '#3c79f2' , fontFamily: 'Gill Sans', fontSize: 36, fontWeight: 600, marginBottom: '60px'}}>Install Git</Text>
