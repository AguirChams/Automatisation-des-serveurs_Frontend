...
useEffect(() => {
  ...
  const mqttTopic = 'executionTerminatedJava';
  ...
  const handleConnect = () => {
    ...
    client.subscribe('executionTerminatedJava', { qos: 0 }, (err) => {
      if (err) {
  ...
  const handleExecuteScript = async () => {
    try {
      const res = await axios.post('/api/java/executeCommand');
  ...
  return (
    ...
    <Col md={10}>
          <View style={{ display: 'block', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: '30%', marginLeft: '15%' }}>
            ...
            <Text style={{ textAlign: 'center', marginBottom: 20, color: '#3c79f2' , fontFamily: 'Gill Sans', fontSize: 36, fontWeight: 600, marginBottom: '60px'}}>Install Java</Text>
