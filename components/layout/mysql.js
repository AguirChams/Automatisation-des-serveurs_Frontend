...
const UpdateScript = () => {
  ...
  const handleSubmit = async () => {
    const data = { db_user:db_user, db_pass:db_pass, db_name:db_name };
    try {
      setLoading(true); 
      const res = await axios.post(`/api/mysql/addMysql`,data);
  ...
  const handleExecuteScript = async () => {
    const data = { db_user: db_user, db_pass:db_pass, db_name:db_name};
    try{    
    const res1 = await axios.post('/api/mysql/executeMysql', data);
    ...
    const handleExecutePlaybook = async () => {
      try{    
        const res2 = await axios.post('/api/mysql/executeCommand');
    ...
    useEffect(() => {
      ...
      const mqttTopic = 'executionTerminatedMysql';
  ...
  return (
   ...
   <View style={{ display: 'block', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: '30%', marginLeft: '15%' }}>
            ...
            <Text style={{ textAlign: 'center', marginBottom: 20, color: '#3c79f2' , fontFamily: 'Gill Sans', fontSize: 36, fontWeight: 600, marginBottom: '60px'}}>Mysql Database</Text>
        <Form onSubmit={handleSubmit} style={{
          ...
          >
            ...
            <Form.Item>
                  ...
                  }} style={{ width: '30%' }}>Install MySQL Database</Button>
            </Form.Item>
            <Form.Item>
