...
const UpdateScript = () => {
  ...
  const handleExecuteScript = async () => {
    try {
      const res = await axios.post('/api/nginx/executeCommand');
  ...
  useEffect(() => {
    ...
    const mqttTopic = 'executionTerminatedNginx';
    ...
    const handleConnect = () => {
      ...
      client.subscribe('executionTerminatedNginx', { qos: 0 }, (err) => {
        if (err) {
...
useEffect(() => {
  ...
  return (
    ...
    <View style={{ display: 'block', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: '30%', marginLeft: '15%' }}>
            ...
            <Text style={{ textAlign: 'center', marginBottom: 20, color: '#3c79f2' , fontFamily: 'Gill Sans', fontSize: 36, fontWeight: 600, marginBottom: '60px'}}>Install Nginx Web Server</Text>
