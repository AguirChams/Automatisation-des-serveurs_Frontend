...
const UpdateScript = () => {
  ...
  const [jenkins_admin_username, setjenkins_admin_username] = useState('');
  const [jenkins_admin_password, setjenkins_admin_password] = useState('');
  ...
  const handleSubmit = async () => {
    const data = { key: key, jenkins_admin_password: jenkins_admin_password,jenkins_admin_username: jenkins_admin_username  };
    try {
      setLoading(true); 
      const res = await axios.post(`/api/jenkins/addJenkins`,data);
  ...
  const handleExecuteScript = async () => {
    const data = { key: key , jenkins_admin_password: jenkins_admin_password,jenkins_admin_username: jenkins_admin_username};
    try{    
    const res1 = await axios.post('/api/jenkins/executeJenkins', data);
    ...
    const handleExecutePlaybook = async () => {
      try {
        const res2 = await axios.post('/api/jenkins/executeCommand');
    ...
    useEffect(() => {
      ...
      const mqttTopic = 'executionTerminatedJenkins';
      ...
      const handleConnect = () => {
        ...
        client.subscribe('executionTerminatedJenkins', { qos: 0 }, (err) => {
          if (err) {
  ...
  return (
   ...
   <Col md={10}>
    <View style={{ display: 'block', justifyContent: 'center', alignItems: 'center', height: '100%',marginTop: '30%', marginLeft: '10%'  }}>
      ...
      <Text style={{ textAlign: 'center', marginBottom: 20, color: '#3c79f2' , fontFamily: 'Gill Sans', fontSize: 36, fontWeight: 600, marginBottom: '60px'}}>Jenkins</Text>
        <Form onSubmit={handleSubmit} style={{
          ...
          <Form.Item 
            ...
            name="Key"              
            rules={[
              { required: true, message: 'Please enter Jenkins repository key' },
            ]}>
          ...
          <Form.Item 
            ...
            rules={[
              { required: true, message: 'Please enter Jenkins admin username' },
            ]}>
            <Input value={ key } onChange={(e) => setKey(e.target.value)} style={{
                      background: 'rgba(255, 255, 255, .3)',
                      borderRadius: 20,
          ...
          <Form.Item 
            ...
            rules={[
              { required: true, message: 'Please enter Jenkins admin password' },
            ]}>
            <Input value={ key } onChange={(e) => setKey(e.target.value)} style={{
                      background: 'rgba(255, 255, 255, .3)',
                      borderRadius: 20,
