...
const UpdateScript = () => {
  ...
  useEffect(() => {
    ...
    const mqttTopic = 'executionTerminatedRemove';
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const response = await axios.get('/api/remove/userProfiles');
  ...
  const handleExecuteScript = async () => {
    ...
    try {
      const res1 = await axios.post('/api/remove/executeScript', data);
  ...
  const handleExecutePlaybook = async () => {
    try{    
      const res2 = await axios.post('/api/remove/executeCommand');
    ...
    <View
          ...
          >
            ...
            <Form.Item
              ...
              name="user"
              rules={[{ required: true, message: 'Please enter the username of your target host (exp: node1, node2 ..).' }]}
              initialValue={user}
            ...
            <Button type="button" onClick={() => {
                    handleExecuteScript();
                  }} style={{ width: '100%' }}>Save</Button></Form.Item>
                  <Form.Item  style={{ display: 'flex', justifyContent: 'center' }}>
                  <Button type="button" onClick={() => {
